...
def save_triggerset(trigger_set, path, runname, wm_type=None):
    logging.info('Saving watermarks.')
    os.makedirs(path, exist_ok=True)

    # path = os.path.join(path, runname)
    # if wm_type:
...
def set_up_optim_sched(args, net):
    ...
    if args.sched == 'MultiStepLR_cifar10_0':
        scheduler = MultiStepLR(optimizer, milestones=[10, 20, 30, 40, 50, 60, 70], gamma=args.lradj)
    elif args.sched == 'MultiStepLR_cifar10_1':
        scheduler = MultiStepLR(optimizer, milestones=[20, 40, 60, 80], gamma=args.lradj)
    elif args.sched == 'MultiStepLR_mnist_0':
        scheduler = MultiStepLR(optimizer, milestones=[5, 10, 15, 20, 25, 30], gamma=args.lradj) # mnist fashionmnist
    elif args.sched == 'MultiStepLR_mnist_1':
        scheduler = MultiStepLR(optimizer, milestones=[10, 20, 30], gamma=args.lradj) # mnist fashionmnist
...
def wm_regularizer_loss(net, b, X, device, model):
    b_tensor = torch.tensor(b).to(device)
    X_tensor = torch.tensor(X).to(device)
    
    if model == 'cnn_mnist' or model == 'cnn_cifar10':
        for name, param in net.named_parameters():
            if 'conv_layer.6.weight' in name:
                w = param.reshape(128, 576).mean(0)
    elif model == 'resnet18':
        for name, param in net.named_parameters():
            if 'layer2.0.conv1.weight' in name:
                w = param.reshape(128, 576).mean(0)
    
    y_tensor = nn.Sigmoid()(torch.matmul(X_tensor.float(), w))
    wm_regularizer_loss = nn.BCELoss()(y_tensor, b_tensor.float())
    return wm_regularizer_loss