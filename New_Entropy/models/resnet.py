            ...
            if isinstance(module, torch.nn.Sequential):
                for sub_module in module:
                    if isinstance(sub_module, BasicBlock):
                        sub_module.conv1.enable(t)
                        sub_module.conv2.enable(t)
                    elif isinstance(sub_module, Bottleneck):
                        sub_module.conv1.enable(t)
                        sub_module.conv2.enable(t)
                        sub_module.conv3.enable(t)
            elif isinstance(module, EWConv2d) or isinstance(module, EWLinear):
                module.enable(t)   
    ...
    def disable_ew(self):
        for i, module in self._modules.items():
            if isinstance(module, torch.nn.Sequential):
                for sub_module in module:
                    if isinstance(sub_module, BasicBlock):
                        sub_module.conv1.disable() 
                        sub_module.conv2.disable() 
                    elif isinstance(sub_module, Bottleneck):
                        sub_module.conv1.disable() 
                        sub_module.conv2.disable() 
                        sub_module.conv3.disable() 
            elif isinstance(module, EWConv2d) or isinstance(module, EWLinear):
                module.disable()  
