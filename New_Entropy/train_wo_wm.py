...
parser.add_argument('--arch', metavar='ARCH', default='lenet5', choices=model_names,
                    help='model architecture: ' + ' | '.join(model_names) + ' (default: )')

# hyperparameters
parser.add_argument('--runname', default='test', help='the exp name')
...
try:
    ...
    args.runname = f"{str(args.dataset)[0].upper()}_{str(args.arch)}_{str(args.lr)}_{str(args.sched).split('_')[2]}_{str(args.epochs_wo_wm)}_{str(args.runname)}"

    # create log and config file
    log_dir = os.path.join(cwd, 'log', str(args.dataset), str(args.arch), str(args.lr), str(args.sched), str(args.epochs_wo_wm))
    ...
    save_dir = os.path.join(cwd, 'checkpoint', str(args.dataset), str(args.arch), str(args.lr), str(args.sched), str(args.epochs_wo_wm))
