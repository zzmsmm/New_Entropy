...
try:
    ...
    if args.select == 'random':
        if args.R:
            os.makedirs(os.path.join(cwd, 'result', 'R_index', str(args.dataset), 'random_select'), exist_ok=True)
            save_path = os.path.join(cwd, 'result', 'R_index', str(args.dataset), 'random_select', test_method + '.txt')

            model_list = range(600)
            for N in N_list:
                for epsilon in epsilon_list:
                    trigger_list = range(N)

                    epsilon_float = "{:.3f}".format(epsilon)
                    r_rate = "{:.3f}".format(module_r(phi_matrix, model_list, trigger_list, epsilon))
            ...
            # randomly select
            os.makedirs(os.path.join(cwd, 'result', 'I_index', str(args.dataset), 'random_select'), exist_ok=True)
            save_path = os.path.join(cwd, 'result', 'I_index', str(args.dataset), 'random_select', test_method + '.txt')

            result = np.array([])

            model_list = range(600)
            for i in range(50):
                trigger_list = range(i)
                result = np.append(result, module_i(phi_matrix, model_list, trigger_list, i))
            
            np.savetxt(save_path, result, fmt='%.10f')
    
    elif args.select =='greedy':
        if args.R:
            os.makedirs(os.path.join(cwd, 'result', 'R_index', str(args.dataset), 'greedy_select'), exist_ok=True)
            save_path = os.path.join(cwd, 'result', 'R_index', str(args.dataset), 'greedy_select', test_method + '.txt')
            ...
            for N in N_list:
                for epsilon in epsilon_list:
                    ...
                    while len(choose_list) < N:
                        ...
                        r = 0
                        for i in left_list:
                            h = module_i(phi_matrix, model_list, choose_list, i)
                            if h > hmax:
                                hmax = h
                                r = i
            ...
            while len(left_list) > 0:
                ...
                for epsilon in epsilon_list:
                    ...
                    with open(save_path, "a") as file:
                        file.write(f"{len(choose_list)}\t{epsilon_float}\t{r_rate}\n")

        else:
            # greedily select
            os.makedirs(os.path.join(cwd, 'result', 'I_index', str(args.dataset), 'greedy_select'), exist_ok=True)
            save_path = os.path.join(cwd, 'result', 'I_index', str(args.dataset), 'greedy_select', test_method + '.txt')
            save_path2 = os.path.join(cwd, 'result', 'I_index', str(args.dataset), 'greedy_select', test_method + '-choose_list.txt')
            ...
            while len(left_list) > 0:
                ...
                choose_list.append(r)
                left_list.remove(r)
                result = np.append(result, hmax)
                print(len(left_list))

            np.savetxt(save_path, result, fmt='%.10f')
            np.savetxt(save_path2, choose_list, fmt='%d') 
